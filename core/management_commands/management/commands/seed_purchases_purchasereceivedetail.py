...
from purchases.models import PurchaseReceiveHeader, PurchaseReceiveDetail
from products.models import ProductVariation


class Command(BaseCommand):
    help = "Insert purchase receive details from an Excel file."

    def add_arguments(self, parser):
        parser.add_argument(
            'filename', type=str, help="The Excel file (XLSX) containing purchase receive details.")
    ...
    def handle(self, *args, **options):
        ...
        # Validate that HEADER CODE exists in PurchaseReceiveHeader
        header_codes = set(df['HEADER CODE'])
        existing_headers = set(PurchaseReceiveHeader.objects.filter(
            code__in=header_codes).values_list('code', flat=True))
        missing_headers = header_codes - existing_headers

        if missing_headers:
            raise CommandError(f"Invalid HEADER CODE(s) (not found in PurchaseReceiveHeader): {
        ...
        with transaction.atomic():
            for _, row in df.iterrows():
                header = PurchaseReceiveHeader.objects.get(
                ...
                PurchaseReceiveDetail.objects.create(
                    purchase_receive_header=header,
        ...
        self.stdout.write(self.style.SUCCESS(
            "purchase receive Details data import completed!"))
