...
from sales.models import OfficialReceiptHeader, OfficialReceiptDetail
from products.models import ProductVariation


class Command(BaseCommand):
    help = "Insert official receipt details from an Excel file."

    def add_arguments(self, parser):
        parser.add_argument(
            'filename', type=str, help="The Excel file (XLSX) containing official receipt details.")
    ...
    def handle(self, *args, **options):
        ...
        # Validate that HEADER CODE exists in OfficialReceiptHeader
        header_codes = set(df['HEADER CODE'])
        existing_headers = set(OfficialReceiptHeader.objects.filter(
            code__in=header_codes).values_list('code', flat=True))
        missing_headers = header_codes - existing_headers

        if missing_headers:
            raise CommandError(f"Invalid HEADER CODE(s) (not found in OfficialReceiptHeader): {
        ...
        with transaction.atomic():
            for _, row in df.iterrows():
                header = OfficialReceiptHeader.objects.get(
                ...
                OfficialReceiptDetail.objects.create(
                    official_receipt_header=header,
        ...
        self.stdout.write(self.style.SUCCESS(
            "official receipt Details data import completed!"))
