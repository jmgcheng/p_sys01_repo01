...
class Command(BaseCommand):
    help = "Insert inventory add details from an Excel file."

    def add_arguments(self, parser):
        parser.add_argument(
            'filename', type=str, help="The Excel file (XLSX) containing inventory add details.")
    ...
    def handle(self, *args, **options):
        ...
        # Validate that HEADER CODE exists in InventoryAddHeader
        header_codes = set(df['HEADER CODE'])
        existing_headers = set(InventoryAddHeader.objects.filter(
            code__in=header_codes).values_list('code', flat=True))
        missing_headers = header_codes - existing_headers

        if missing_headers:
            raise CommandError(f"Invalid HEADER CODE(s) (not found in InventoryAddHeader): {
        ...
        with transaction.atomic():
            for _, row in df.iterrows():
                header = InventoryAddHeader.objects.get(
                    code=row['HEADER CODE'])
                variation = ProductVariation.objects.get(
                    code=row['PRODUCT VARIATION CODE'])
                quantity = int(row['QUANTITY'])

                InventoryAddDetail.objects.create(
                    inventory_add_header=header,
        ...
        self.stdout.write(self.style.SUCCESS(
            "Inventory Add Details data import completed!"))
