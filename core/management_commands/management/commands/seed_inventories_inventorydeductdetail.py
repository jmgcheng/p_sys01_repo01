...
from inventories.models import InventoryDeductHeader, InventoryDeductDetail
from products.models import ProductVariation


class Command(BaseCommand):
    help = "Insert inventory deduct details from an Excel file."

    def add_arguments(self, parser):
        parser.add_argument(
            'filename', type=str, help="The Excel file (XLSX) containing inventory deduct details.")
    ...
    def handle(self, *args, **options):
        ...
        # Validate that HEADER CODE exists in InventoryDeductHeader
        header_codes = set(df['HEADER CODE'])
        existing_headers = set(InventoryDeductHeader.objects.filter(
            code__in=header_codes).values_list('code', flat=True))
        missing_headers = header_codes - existing_headers

        if missing_headers:
            raise CommandError(f"Invalid HEADER CODE(s) (not found in InventoryDeductHeader): {
        ...
        with transaction.atomic():
            for _, row in df.iterrows():
                header = InventoryDeductHeader.objects.get(
                    code=row['HEADER CODE'])
                variation = ProductVariation.objects.get(
                    code=row['PRODUCT VARIATION CODE'])
                quantity = int(row['QUANTITY'])

                InventoryDeductDetail.objects.create(
                    inventory_deduct_header=header,
        ...
        self.stdout.write(self.style.SUCCESS(
            "Inventory Deduct Details data import completed!"))
